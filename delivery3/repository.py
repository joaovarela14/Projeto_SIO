...
def verify_request(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not received_hmac:
            return jsonify({"error": "Missing HMAC"}), 400

        session_id = data.get("session_id")
        if not session_id or not received_hmac:
            return jsonify({"error": "Missing session_id or hmac"}), 400
        ...
        # Serialize payload for HMAC verification
        ...
        # For state-changing requests, verify and update counter

        expected_counter = session_data.get("last_counter", 0) + 1
        if data["counter"] != expected_counter:
            return jsonify({"error": "Invalid counter value"}), 403

        session_data["last_counter"] = data["counter"]
