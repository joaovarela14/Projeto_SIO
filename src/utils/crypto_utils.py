...
# === API encryption ===
def encrypt_payload(session_key: bytes, payload_dict: dict) -> dict:
    """
    Encrypt `payload_dict` (as JSON) using AES-GCM with the given session_key.
    Returns a dict of { iv, tag, data }, all base64-encoded.
    """
...
def decrypt_payload(session_key: bytes, encrypted_dict: dict) -> dict:
    """
    Inverse of encrypt_payload on the client side.
    Encrypted_dict is { "iv": base64IV, "tag": base64Tag, "data": base64Ciphertext }.
    Returns the original plaintext JSON as a dict.
    """
    iv = base64.b64decode(encrypted_dict["iv"])
    tag = base64.b64decode(encrypted_dict["tag"])
    ciphertext = base64.b64decode(encrypted_dict["data"])
...
def decrypt_payload_aes(iv, encrypted_payload, encrypted_aes_key, private_key):
