...
def rep_create_org(org, username, name, email, public_key_file):
    ...
    aes_key, iv, encrypted_payload = encrypt_with_aes(payload_json)
    try:
        encrypted_aes_key = encrypt_aes_key_with_rsa(SERVER_PUBLIC_KEY, aes_key)
    except Exception as e:
        print(f"Failed to encrypt AES key: {e}")
...
def rep_list_orgs():
    ...
    # não precisa de ter a chamada encriptada porque não envia nada nela
...
def rep_add_doc(session_file, document_name, file_path):
    ...
    hmac_signature = compute_hmac(session_key, serialized_payload)

    # Add the HMAC to the payload
    payload["hmac"] = hmac_signature

    # Encrypt the payload
    encrypted_payload = encrypt_payload(session_key, payload)
...
def rep_get_doc_file(session_file, document_name, output_file=None):
    ...
    try:
        response = requests.get(url, json=encrypted_payload, headers={"X-Session-ID": session_id})
        if response.status_code == 200:
            data = response.json()
...
def rep_suspend_subject(session_file, username):
    ...
    encripted_payload = encrypt_payload(session_key, payload)

    try:
        response = requests.post(url, json=encripted_payload, headers={"X-Session-ID": session_id})
        if response.status_code == 200:
...
def rep_get_doc_metadata(session_file, document_name):
    ...
    url = f"http://127.0.0.1:5000/documents/{document_name}/metadata"
    params = {
        "session_id": session_id,
        "counter": get_and_increment_counter(session_file)
    }

    serialized_payload = json.dumps(params, sort_keys=True)
    hmac_signature = compute_hmac(session_key, serialized_payload)
    params["hmac"] = hmac_signature        

    encrypted_payload = encrypt_payload(session_key, params) 

    try:
        response = requests.post(url, json=encrypted_payload, headers={"X-Session-ID": session_id})
        if response.status_code == 200:
...
def rep_get_file(file_handle, output_path=None):
    ...
    """
    encoded_handle = quote(file_handle, safe="")   

    url = f"http://localhost:5000/file/{encoded_handle}"
...
def rep_delete_doc(session_file, document_name):
    ...
    try:
        response = requests.delete(url, json=encripted_payload, headers={"X-Session-ID": session_id})
        if response.status_code == 200:
            data = response.json()
...
def rep_list_docs(session_file, username=None, date=None, date_filter_type=None):
    ...
    params["counter"] = get_and_increment_counter(session_file)
    ...
    encripted_payload = encrypt_payload(session_key, params)
...
def rep_list_roles(session_file):
    ...
    session_key_bites = base64.b64decode(session_key)
    ...
    serialized_payload = json.dumps(payload, sort_keys=True)
    hmac_signature = compute_hmac(session_key_bites, serialized_payload)
    payload["hmac"] = hmac_signature

    encripted_payload = encrypt_payload(session_key_bites, payload)

    url = f"http://127.0.0.1:5000/session/roles"
    try:
        response = requests.get(url, json=encripted_payload, headers={"X-Session-ID": session_id})
        if response.status_code == 200:
            data = response.json()
...
def rep_list_role_permissions(session_file, role):
    ...
    url = f"http://127.0.0.1:5000/role/{role}/permissions"
    try:
        resp = requests.get(url, json=encripted_payload, headers={"X-Session-ID": session_id})
...
def rep_list_permission_roles(session_file, permission):
    ...
    url = f"http://127.0.0.1:5000/permission/{permission}/roles"
...
def rep_suspend_role(session_file, role):
    ...
    payload = {"session_id": session_id, "role": role, "counter": get_and_increment_counter(session_file)}
    ...
    try:
        resp = requests.post(url, json=encripted_payload, headers={"X-Session-ID": session_id})
        if resp.status_code == 200:
...
def rep_list_subject_roles(session_file, username):
    ...
    url = f"http://127.0.0.1:5000/subject/{username}/roles"
...
def rep_list_role_subjects(session_file, role):
    ...
    url = f"http://127.0.0.1:5000/role/{role}/subjects"
...
def rep_remove_permission(session_file, role, arg):
    ...
    payload["counter"] = get_and_increment_counter(session_file)
    serialized_payload = json.dumps(payload, sort_keys=True)
    hmac_signature = compute_hmac(session_key, serialized_payload)
    payload["hmac"] = hmac_signature
